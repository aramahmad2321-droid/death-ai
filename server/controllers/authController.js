...
exports.demoLogin = async (req, res, next) => {
  try {
    let demoUser = null;
    try {
      demoUser = await User.findOne({ email: 'demo@zana-ai.com' });
      if (!demoUser) {
        demoUser = new User({
          name: 'Demo User',
      ...
      // Fallback mock user if database is offline
      demoUser = {
        _id: '6a6207597bff8983879f424a',
        name: 'Demo User',
        email: 'demo@zana-ai.com',
        role: 'user',
        theme: 'dark',
        language: 'ku',
        toSafeObject: function() { return { _id: this._id, name: this.name, email: this.email, role: this.role, theme: this.theme, language: this.language }; }
      };
    }
